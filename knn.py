...
# Dinamik yol - training_outputs klasöründen veri oku
...
# bu kısım üçüncü sefer aldığımız hatanın kaynağı:
# column_names hesaplaması yanlış! 
# 6 katman × 2 model = 12 sütun adı oluşturuyor ama sr sadece 6 sütunlu.
    # # Set column names
    # column_names = np.repeat(np.arange(1, len(scalar_curvs1[0]) + 1), len(np.where(acc > a)[0]))

    
    # Set column names - düzeltme: sadece katman sayısı kadar sütun adı oluştur
    column_names = np.arange(1, len(scalar_curvs1[0]) + 1)
   
   
    # ikinci sefer aldığımız hatanın kaynağı bu kısım
    # Debug: print shapes to understand the issueiğer 
    print(f"sr shape: {sr.shape}")
    print(f"column_names shape: {column_names.shape}")
    print(f"len(scalar_curvs1[0]): {len(scalar_curvs1[0])}")
    print(f"len(np.where(acc > a)[0]): {len(np.where(acc > a)[0])}")
    # Create DataFrame with correct column names
    df = pd.DataFrame(sr, columns=column_names)
    ...
    # Debug: print shapes - boyut uyumsuzluğunu tespit etmek için eklendi
    # column_stack hatası: 2000 vs 20000 boyut uyumsuzluğu var
    print(f"ssr shape: {ssr.shape}")
    print(f"x_test.shape[0]: {x_test.shape[0]}")
    print(f"len(scalar_curvs1[0]): {len(scalar_curvs1[0])}")
    print(f"len(np.where(acc > a)[0]): {len(np.where(acc > a)[0])}")
    
    # Layer bilgilerini oluştur - her katman için tekrarlanan değerler
    ...
    # Model bilgilerini oluştur - her model için tekrarlanan değerler
    ...
    # Debug: print final shapes - boyut uyumsuzluğunun kaynağını bulmak için
...
# bu kısımda layer matching hatası var.
